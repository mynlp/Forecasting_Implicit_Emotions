...
def make_dataset(mode, random_state):
    ...
    if mode == "full-history":
        ...
        df.rename(columns={"history": "text", "next_uttr_emotion": "label"}, inplace=True)
    elif mode == "last-uttr":
        df = deepcopy(emotions)
        ...
        df = deepcopy(emotions)
        df.rename(columns={"utterance": "text", "next_uttr_emotion": "label"}, inplace=True)
        
    df["label"].replace({"positive": 0, "neutral": 1, "negative": 2}, inplace=True)
    df_train = df[df["split"]=="train"].sample(4767, random_state=random_state)
...
for mode in ["full-history", "last-uttr", "no-history"]:
    ...
    for i in range(5):
        ...
        label_ratio, emotions_dataset = make_dataset(mode, i)
        emotions_encoded = emotions_dataset.map(tokenize, batched=True)

        training_args = TrainingArguments(
            output_dir=f"EEFC-{model_ckpt}-{mode}-4767-{i}",
        ...
        trainer.train()
        trainer.save_model(f"EEFC-{model_ckpt}-{mode}-4767-{i}")
